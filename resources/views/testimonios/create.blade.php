<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('testimonios.store') }}" method="POST" class="space-y-6">
                @csrf
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="comentario" id="comentario" rows="5" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('comentario') }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="fuente" class="block text-sm font-medium text-gray-700">Fuente</label>
                                <input type="text" name="fuente" id="fuente" value="{{ old('fuente') }}"
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_publicacion" id="fecha_publicacion" value="{{ old('fecha_publicacion', date('Y-m-d')) }}" required
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                        </div>
