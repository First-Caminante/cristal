                ...
                <div class="p-6">
                    <form action="{{ route('usuarios.buscar') }}" method="GET" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label for="tipo_busqueda" class="block text-sm font-medium text-gray-700">Buscar
                                    por:</label>
                            ...
                            <div>
                                <label for="termino" class="block text-sm font-medium text-gray-700">Término de
                                    búsqueda:</label>
                                <input type="text" name="termino" id="termino" value="{{ $termino ?? '' }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                        ...
                                        @if(auth()->user()->isSuperAdmin())
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                ...
                                                @if($usuario->id !== auth()->id())
                                                    ...
                                                    <form action="{{ route('usuarios.toggle-estado', $usuario) }}" method="POST"
                                                        class="inline">
                                                        @csrf
                                                        @method('PATCH')
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-4">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center">
                                    <div
                                        class="h-12 w-12 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold mr-3">
                                        {{ strtoupper(substr($usuario->nombre, 0, 1)) }}{{ strtoupper(substr($usuario->apellido, 0, 1)) }}
                                    </div>
                                    <div>
                                    ...
                                    @if($usuario->id !== auth()->id())
                                        ...
                                        <form action="{{ route('usuarios.toggle-estado', $usuario) }}" method="POST" class="flex-1">
                                            ...
                                            <button type="submit"
                                                ...
                                                {{ $usuario->estado ? 'Desconectar' : 'Conectar' }}
                                            </button>
                                        </form>
