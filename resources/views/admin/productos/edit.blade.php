    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('productos.admin.update', $producto) }}" method="POST"
                            ...
                            @if($producto->imagen)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $producto->imagen) }}" alt="{{ $producto->nombre }}"
                                        class="h-32 w-32 object-cover rounded-md shadow-sm">
                                    <p class="text-xs text-gray-500 mt-1">Imagen actual</p>
                                </div>
                            @endif
                            <input type="file" name="imagen" id="imagen"
                                class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                            <p class="text-xs text-gray-500 mt-1">Deje vacío para mantener la imagen actual. Formatos:
                                JPG, PNG, WEBP.</p>
                            @error('imagen')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <div id="caracteristicas-container" class="space-y-2">
                                @forelse(old('caracteristicas', $producto->caracteristicas ?? []) as $caract)
                                    <div class="flex items-center space-x-2">
                                        <input type="text" name="caracteristicas[]" value="{{ $caract }}"
                        ...
                        <div class="mb-4">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="estado"
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('estado', $producto->estado) ? 'checked' : '' }}>
